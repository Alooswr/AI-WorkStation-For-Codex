---
name: pua
description: "失败 2 次以上或用户明确要求强制排障时使用的兜底排障激励 Skill。Use only when: (1) 同一任务已失败 2+ 次或反复微调同一思路; (2) 用户明确要求 'try harder'、'stop giving up'、'换个方法'、'为什么还不行'、'你再试试'、'你怎么又失败了'; (3) 即将说'我无法解决'、建议用户手动操作、或未验证就归因环境。Do not use for normal first-pass work, routine implementation, already-known fixes in progress, or tasks that have a clear domain skill."
license: MIT
---
...
这个 skill 是**兜底强制排障工具**，只在任务失败 2 次以上、用户明确要求强制排障，或模型即将放弃/甩锅时触发。正常首轮任务、已有明确修复路径的任务、或已有清晰领域 Skill 的任务，不要加载它。

## 触发边界

Use when:

- 同一任务已经失败 2 次以上，或同一思路反复微调仍无进展。
- 用户明确要求："try harder"、"stop giving up"、"换个方法"、"为什么还不行"、"你再试试"、"你怎么又失败了"。
- 即将说"我无法解决"、建议用户手动操作，或未验证就归因环境。

Do not use when:

- 正常首轮分析、实现、阅读、整理、写作或例行验证。
- 已知修复正在执行，只是还没跑完验证。
- 已有更精确的领域 Skill 可以处理当前任务。
