...
全书鸟瞰。顶部为「全书体量与阶段总览」（总章节数/目标字数/全书情绪曲线/阶段划分/各阶段节奏公式/关键节点与钩子链，结构见 [Phase 3：大纲搭建](../workflow.md#phase-3大纲搭建)），其后接卷级大纲一段式汇总：
...
细纲是正文写作的章节蓝图，而不是只列事件和钩子的清单。**模板唯一权威副本**见 [Phase 3：大纲搭建](../workflow.md#phase-3大纲搭建) 的「细纲（全书每章）」模板（含单元ID/位置、主角目标/关键选择、任务卡点、结构公式、契约风险行、密/疏字数预算与「预算合计」行）；本文件不再保留模板副本，避免双模板漂移。旧项目已有 `核心事件 / 情节点序列 / 目标情绪 / 章首钩子 / 爽点 / 章尾钩子 / 字数目标` 时仍可日更、不阻塞；新建、补建、回填按权威模板补齐，未知字段写 `[待补充]`，不要为了补齐字段杜撰副线或人物关系。契约/推进字段只加在章纲摘要层，长规则统一参 `reader-contract-and-progression.md`。
