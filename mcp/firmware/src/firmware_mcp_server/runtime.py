...
from .config import STANDARD_ACTIONS, STANDARD_ACTION_SET, CommandConfig, DeviceConfig, DeviceRegistry
...
from .log_normalizer import classify_log_line, normalize_command_logs, normalize_serial_logs
...
class ToolExecutor:
    def __init__(self, runtime: DeviceRuntime) -> None:
        ...
        self._handlers: dict[str, ToolHandler] = {
            ...
            "run_project_action": self._run_project_action,
            "list_project_actions": self._list_project_actions,
    ...
    async def _scan_device(self, args: dict[str, Any]) -> dict[str, Any]:
        def select_scanner(device: DeviceConfig) -> CommandConfig:
            if device.scanner is None:
                raise ValidationError(f"device has no scanner.command configured: {device.device_id}")
            return device.scanner

        return await self._run_device_command("scan_device", args, select_scanner)

    async def _run_project_action(self, args: dict[str, Any]) -> dict[str, Any]:
        action = require_project_action(args)
        if action == "monitor":
            return await self._run_monitor_action(args)

        def select_action(device: DeviceConfig) -> CommandConfig:
            command = device.actions.get(action)
            if command is None:
                raise ValidationError(f"device has no actions.{action}.command configured: {device.device_id}")
            return command

        return await self._run_device_command(action, args, select_action)

    async def _list_project_actions(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        async with self._runtime.lock_for(device_id):
            ...
            configured = set(device.actions)
            if device.serial is not None:
                configured.add("monitor")

        return success(
            {
                "device_id": device_id,
                "framework": device.framework,
                "standard_actions": list(STANDARD_ACTIONS),
                "configured_actions": [action for action in STANDARD_ACTIONS if action in configured],
                "missing_actions": [action for action in STANDARD_ACTIONS if action not in configured],
                "monitor_source": "command" if "monitor" in device.actions else "serial",
                "scanner_configured": device.scanner is not None,
            }
        )

    async def _run_monitor_action(self, args: dict[str, Any]) -> dict[str, Any]:
        device_id = require_device_id(args)
        ...
        async with self._runtime.lock_for(device_id):
            device = await self._runtime.get_device(device_id)
            command = device.actions.get("monitor")
            if command is not None:
                try:
                    data = await execute_command_action("monitor", device_id, command, timeout_ms)
                except CommandExecutionError as exc:
                    return failure(
                        ToolError(
                            error_type=command_failed_error_type("monitor"),
                            message=exc.message,
                            recoverable=True,
                        )
                    )
            else:
                data = await self._collect_serial_log(device, device_id, duration_ms, max_lines, "monitor")

        if data.get("timed_out"):
            return failure(
                ToolError(
                    error_type="TIMEOUT",
                    message=f"monitor timed out for device: {device_id}",
                    recoverable=True,
                ),
                data=data,
            )
        ...
        if data.get("exit_code", 0) != 0:
            return failure(
                ToolError(
                    error_type=command_failed_error_type("monitor"),
                    message=f"monitor exited with code {data['exit_code']} for device: {device_id}",
                    recoverable=True,
                ),
                data=data,
            )
    ...
    async def _read_serial_log(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        async with self._runtime.lock_for(device_id):
            device = await self._runtime.get_device(device_id)
            data = await self._collect_serial_log(device, device_id, duration_ms, max_lines, "read_serial_log")

        return success(data)

    async def _collect_serial_log(
        self,
        device: DeviceConfig,
        device_id: str,
        duration_ms: int,
        ...
        return {
            "device_id": device_id,
            "action": action,
            "monitor_source": "serial" if action == "monitor" else None,
            "serial": {
            ...
            "logs": normalize_serial_logs(lines),
            "lines": lines,
        }
    ...
    def _record_serial_stream_event(
        ...
        event = {
            ...
            "level": classify_log_line(line["line"]),
            "line": line["line"],
        }
    ...
    def _emit_execution_trace(
        ...
        trace = {
            ...
            "level": "Info" if result.get("ok") is True else "Error",
            "tool_name": tool_name,
            "device_id": device_id,
...
async def execute_command_action(
    ...
    return {
        "device_id": device_id,
        "action": action,
        "logs": normalize_command_logs(result.get("stdout", ""), result.get("stderr", "")),
        **result,
    }
...
def require_project_action(args: dict[str, Any]) -> str:
    action = args.get("action")
    if not isinstance(action, str) or not action.strip():
        raise ValidationError("action must be a non-empty string")
    action = action.strip()
    if action not in STANDARD_ACTION_SET:
...
def command_failed_error_type(action: str) -> str:
    if action in {"build_firmware", "build"}:
        return "BUILD_FAILED"
    if action in {"flash_firmware", "flash"}:
        return "FLASH_FAILED"
    if action in {"reset_device", "reset"}:
        return "RESET_FAILED"
